<template>
    <div class="text-center" >         
        <nav class="navbar navbar-light navbar-sticky sticky-top" style="background-color: #EA5D5D;">
            <div class="container text-center">
                <div class="container-fluid">
                    <span class="navbar-brand mb-0 h1">Aggiungi nuovo codice</span>
                </div>
            </div>
        </nav>
        <div class="my-4">        
            <div class="container">
                <StreamBarcodeReader style="height:60%" class="pt-5 pb-5-5 mb-3 mt-5"
                    @decode="onDecode">
                </StreamBarcodeReader>
            </div>
            <div class="container mt-4">
            <div class="alert alert-info my-3" role="alert">
                <svg class="bi flex-shrink-0 me-2" width="24" height="24" role="img" aria-label="Warning:"><use xlink:href="#exclamation-triangle-fill"/></svg>
                <div>
                Assicurati che ci siano condizioni di luce buone
                </div>
            </div>
                
                <a @click="$router.go(-1)"><button type="button" class="btn btn-danger" style="width:70%;">Annulla</button></a>
                </div>
        </div>
        <symbol id="exclamation-triangle-fill" fill="currentColor" viewBox="0 0 16 16">
                    <path d="M8.982 1.566a1.13 1.13 0 0 0-1.96 0L.165 13.233c-.457.778.091 1.767.98 1.767h13.713c.889 0 1.438-.99.98-1.767L8.982 1.566zM8 5c.535 0 .954.462.9.995l-.35 3.507a.552.552 0 0 1-1.1 0L7.1 5.995A.905.905 0 0 1 8 5zm.002 6a1 1 0 1 1 0 2 1 1 0 0 1 0-2z"/>
        </symbol>
                
    </div>
</template>

<script>
import { readLocalStorage } from '../../public/localStorage.js'
import { StreamBarcodeReader } from "vue-barcode-reader"
export default {
    name: 'scansione',
    components: {
        StreamBarcodeReader
    },
    data(){
        let salvati = readLocalStorage();
        return {
        codici: salvati,
        
        }
    },
    methods: {
        onDecode (result) {
                this.$router.push({name: 'dettagli', params: {contenuto: result, caller: 'scansione', action: this.action, titolo: this.titolo, descrizione: this.descrizione, tipo: this.tipo, imgpreferiti: this.imgpreferiti, imgtipo: this.imgtipo}});
                    //permette lo spostamento quando il codice viene scannerizzati
            }   
     },
     props: {
        caller: String,
        action: String,
        titolo: String, 
        descrizione: String, 
        contenuto: String,
        imgpreferiti: String,
        tipo: String,
        imgtipo: String
    }
}
</script>
